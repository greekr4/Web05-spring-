...
<body>
   <div id="wrap">
    ...
    <section class="main_wrap">
                    ...
                    <!-- 회원가입 -->
                    <div class="joinForm">
                         <form:form modelAttribute="CustomerDTO" action="Edit" method="post" name="join_form" onsubmit="return joinCheck()">
                            <table class="table">
                                <tbody>
                                    <tr>
                                        <th><form:label path="email">이메일</form:label></th>
                                        <td>
                                        <form:input path="email" name="email" id="email" onchange="emailCheck()" disabled="true" value="${DTO2.email }"/>
                                    ...
                                    <tr>
                                        ...
                                        <td><form:input path="name" name="name" id="name" disabled="true" value="${DTO2.name }"/></td>
                                    </tr>
                                    <tr>
                                        <th><form:label path="phonenum">핸드폰 번호</form:label></th>
                                        <td><form:input path="phonenum" value="${DTO2.phonenum }"/></td>
                                    </tr>
                                    <tr>
                                        <th rowspan="3"><form:label path="zipcode">주소</form:label></th>
                                        <td>
                                            <form:input path="zipcode" name="zipcode" id="zipcode" value="${DTO2.zipcode }"/>
                                            <input type="button" id="btn1" class="btn_white" onclick="findAddr()" value="우편번호 검색">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td><form:input path="addr1" name="addr1" id="addr1" value="${DTO2.addr1 }"/></td>
                                    </tr>
                                    <tr>
                                        <td><form:input path="addr2" name="addr2" id="addr2" value="${DTO2.addr2 }"/></td>
                                    </tr>
                                </tbody>
                            </table>
                            <input type="hidden" id="seq" name="seq" value="${DTO2.seq }"/>
                            <input type="hidden" id="grade" name="grade" value="${DTO2.grade }"/>
                            <input type="hidden" id="name" name="name" value="${DTO2.name }"/>
                            <input type="submit" class="btn_black btn" value="수정하기">
